 ...
 * 
 * Compile:   gcc -g -Wall -o histogram histogram.c -lpthread
...
#include <pthread.h>
#include <time.h>
/*void *runner( struct thread_data t ){
      bin = Which_bin( t.data[ p ], t.bin_maxes, t.bin_count, t.min_meas );
      bin_counts[ bin ]++;
}*/
...
int main( int argc, char* argv[] ) {
   ...
   //pthread_attr_t *attr[ data_count ]; 
   //struct thread_data *td[ data_count ];
...
/*	for( i=1;i<=data_count;i++ ){
		pthread_t thread[ i-1 ];
   		pthread_attr_init( &attr[ i-1 ] );
		td[ i-1 ].data=data;
		td[ i-1 ].bin_count=bin_count;
		td[ i-1 ].min_meas=min_meas;
		td[ i-1 ].bin_maxes=bin_maxes;
		pthread_create( &thread[ i-1 ],&attr,runner,( void* )thread_data );
